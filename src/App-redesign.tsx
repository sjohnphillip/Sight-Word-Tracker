...
if (!student) return;
  ...
  const promoteUnknownToDeveloping = (section: string) => {
    ...
    updateStudent({ mastery: newMastery, known: newKnown });
    setStatusMessage(`Unknown words in ${section} moved to Develop.`);
  };

  const promoteDevelopingToSecure = (section: string) => {
    if (!student) return;
    const confirmed = window.confirm(`Move all Develop words in ${section} to Secure?`);
    if (!confirmed) return;
    ...
    updateStudent({ mastery: newMastery, known: newKnown });
    setStatusMessage(`Develop words in ${section} moved to Secure.`);
  };
